...
from src.data.data import TabModel
from src.gui.common.ignore_right_menu import IgnoreRightButtonMenu


class DependGroupWidget(QtWidgets.QListWidget):
    ...
    def __init__(self, data_change):
        ...
        self.setStyleSheet("""
                            ...
                            DependGroupWidget::item:selected {
    ...
    def show_add_dialog(self):
        ...
        if ok:
            ...
            self.add_group(TabModel(text))

    def show_edit_dialog(self, index):
        if index >= len(self.groups) or index < 0:
            return
        group = self.groups[index]
        text, ok = QtWidgets.QInputDialog().getText(self, '', '請輸入目標屬性群組名稱', text=group.name)
        if ok:
            if len(text) == 0:
                QtWidgets.QMessageBox.warning(self, "", "群組名稱不能空白")
                return
